...
前述のとおり、JavaScriptのFetch APIを利用します。Herokuは久々にアクセスするときどうに10秒くらいかかるので、（Herokuサーバへの接続確認も兼ねて）フロントエンド側のページにアクセスしたと同時に、以下に示す``wakeup()``という関数を実行させることでHeroku側のアプリを起動させます。  
