...
今日（2021年11月15日）時点で最新のOpenSiv3D v0.6.3を導入します。バージョンが更新されると要求されるライブラリのバージョンなども変わってきますので、必ず[OpenSiv3Dのリポジトリ](https://github.com/Siv3D/OpenSiv3D){:target="_blank"}のREADMEをご確認ください。
...
前準備ではOpenCV、Boost、gccをソースコードからビルドしてインストールするのでかなり時間がかかります。後半の「OpenSiv3Dライブラリのビルド」と「Sivアプリのビルド」では、ほぼSiv3Dリファレンス通りに進めればOKです。
...
を実行してやれば、その後はOpenCV4もBoost 1.71.0もgcc 9.3.0も普通に``apt``コマンドで導入できます。ところがこの``ubuntu-toolchain``、Ubuntu向けあるため（おそらく）Crostiniでは利用できません。いろいろ試行錯誤してみましたが、自分の環境では無理でした。  
