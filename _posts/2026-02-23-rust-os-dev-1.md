...
- #1 [スケジューラとカーネルスレッドの実装](../../../2026/02/23/rust-os-dev-1.html)（今回）
...
`notify_end_of_interrupt()` は割り込みハンドラに割り込み完了 (EOI: End Of Interrupt) を伝えるための処理です。PIC は EOI を受け取るまで、タイマ割り込みハンドラと同等以下の優先度の割り込みを中断します。ここで yield より先に EOI を送信しているのは、yield に遷移するとコンテキストスイッチが行われるため、ここに戻って来る保証がないためです。
