...
src 内のソースファイルは機能ごとにディレクトリが分かれており、① SoLoud のコアエンジンのソース、② 自分が使用するバックエンド用のソース、③ 自分が使用する機能のソースの3種類が必要になります。基本的にディレクトリ内のすべてのソースファイルをプロジェクトに追加しておく感じになります。  
  
ファイルの配置ですが、今回は Main.cpp があるフォルダに 
