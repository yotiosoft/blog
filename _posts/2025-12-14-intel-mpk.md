...
そこで大事になってくるのが、間違ったメモリにアクセスしてしまったら即座に動作を停止させることでメモリ脆弱性を未然に防止する策、いわば「フェイルセーフな設計」です。あらかじめプログラムがアクセスできるメモリ領域を限定しておき、領域外にアクセスしたらアクセスをトラップさせます。いわゆる「メモリ分離技術」などと呼ばれる手法です。

メモリ分離技術の方式はいくつかあります。代表的なものを挙げると、
...
SFI はソフトウェアによってメモリを分離する方法です。プログラムをいくつかのサンドボックスに分割して、メモリアクセス命令に境界チェックを挿入したり、メモリアドレスに対してビットマスクを挿入したりしてアクセスできる範囲を限定します。コンパイラによって境界チェックを自動挿入する手法が多いです。WASM などがこれに該当します。

コンテナはカーネルによってサンドボックス間を分離する方法です。プロセス単位でネットワークなどのリソースを分離する方式が多いです。基本的に OS カーネルの実装に依存します。つまり、カーネル側にバグがあればコンテナ間リークが発生します。有名なのは Docker などがありますね。

VM はもはや説明不要だと思います。基本的に OS ごと分離されているという点で分離強度は一番強いです。ハードウェアの上でハイパバイザが動き、その上でゲスト OS が動く Type1 と、ホスト OS の上でハイパバイザが動き、その上でゲスト OS が動く Type2 があります。

そして、今回の主題が「ハードウェアメモリ保護」の手法です。ハードウェア、すなわち CPU や MMU によるメモリ保護機能を利用して、（一般的には）一つのアドレス空間内に複数のサンドボックスを作成します。主な特長は2つあり、1つはユーザ空間内に複数個のサンドボックスを作成できるので高速なアクセス制御が可能であること、もう1つはハードウェアレベルでの強力なアクセス制御が利用できることが挙げられます。（ハードウェアレベルで制御されるという点では Type1 の VM と同じですが、一般的な呼称が存在しないため悪しからず…）
...
今回は Intel 向けのハードウェアメモリ保護である「**Intel MPK (Memory Protection Keys)**」を扱います。正確には、ユーザ空間向けの「**Intel PKU (Protection Keys for Userspace)」です。近年の Intel CPU（2015年発売の Skylake 世代移行）であれば基本的に搭載されている機能ですので、実は Intel CPU であれば気軽に遊べます。（とは言っても今回は実行環境の都合上、筆者の環境では QEMU 仮想を利用するんですが…）

## Protection Key

Intel MPK/PKU では「Protection Key (PKEY)」と呼ばれるメモリドメインが16個提供されています。メモリドメインとはアクセス制御の単位です。

例えば、あるメモリ領域 0x10000000～0x11000000 に対して Protection key（仮に PKEY 1）を割り当てたとき、そのメモリ領域を一括で read-only にしたり、read/write を許可したり、read/write のいずれも禁止したりすることができます。もし read/write 禁止にしたうえで 0x10000000 のメモリオブジェクトへのアクセスしようとした場合、メモリアクセス命令が実行される段階でハードウェアが実行をトラップし、不正なメモリアクセスを未然に防止することができるわけです。もちろん、バッファオーバーフローや out-of-bounds access などにも有効ですし、それらのメモリ脆弱性を突いた攻撃による影響を緩和できます（完全に防止できるかどうかは、ケースバイケースですが）。
...
ですが、アセンブリでも単純にレジスタに値をセットして命令を呼び出すだけですので、比較的簡単に実装できます。
今回は glibc を使わずに、自分でラッパ関数を実装する方法を下記に示します。（glibc 関数の使い方については、上記リンクをご覧ください）
...
![image-20251214001712842](../assets/img/post/2025-11-12-introduce-mprotect-rs/image-20251214001712842.png)
