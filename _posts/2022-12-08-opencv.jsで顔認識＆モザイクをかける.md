...
まだ未完成なので公開していませんが、以前、Yapps用に写真から顔認識を行い、顔の部分をモザイク加工する「顔モザイク加工」という Web アプリを作ってました。顔認識には JavaScriptによるOpenCVの実行を実現したopencv.jsを利用しています。opencv.jsはwasmによってブラウザ上での仮想環境を実現しており、jsファイルを読み込むだけでOpenCVの機能が利用可能です。  
...
本来なら（公式リファレンスに従ってやるなら） 自前でコンパイルして opencv.js を用意するのですが、面倒なので OpenCV の ドキュメントのサンプルで読み込まれているファイルを拝借しました。  
...
7. 検出した顔領域を赤枠で囲み、検出結果表示用キャンバスに書き出し
...
実際に動かしてみると、こんな感じ。  
![スクリーンショット 2022-12-08 16.06.48](../assets/img/post/2022-12-08/スクリーンショット 2022-12-08 16.06.48.png)  
...
従来なら裏でサーバを動かしてやるような処理ですが、opencv.js は WASM で動いているので、ここまでのことがサーバレスで完結できるところが嬉しいですね。当然 GitHub Pages でも動かせます。  
